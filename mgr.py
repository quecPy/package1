...
m_weather_state =mdls.screen_size_style[mdls.models][14]
BATTERY_OCV_TABLE = {
    "nix_coy_mnzo2": {
        55: {
...
volume_level = mdls.volume_levels[mdls.screen_size_style[mdls.models][7]]
adc_vol_level = mdls.adc_volume_levels[mdls.screen_size_style[mdls.models][7]]
...
class ConfigStoreManager(Abstract):
    def __init__(self):
        ...
        self.lock = Lock()
        self.map = dict(
            media_vol=mdls.screen_size_style[mdls.models][7],
...
class DeviceInfoManager(Abstract):
    ...
    def post_processor_after_instantiation(self):
        ...
        EventMesh.subscribe("ptt_battery_state", self.press_ptt_battery_state)
        ...
        # poc.speak_mode(1)
        # poc.ptt_cfg(mdls.screen_size_style[mdls.models][13][0], mdls.screen_size_style[mdls.models][13][1],
        #             mdls.screen_size_style[mdls.models][13][2])

    def press_ptt_battery_state(self, topic, data):
        '''按下PTT  接收PTT 电池电量状态'''
        if data:
            self.battery_push_schedule.stop()
            # print("PPT按下，停止电量刷下")
        else:
            # ptt抬起  ，接收接收
            self.battery_push_schedule.start(3000, 1, self.__check_battery)
    ...
    def __check_battery(self, *args):
        if EventMesh.publish("get_battery"):
            _bat = EventMesh.publish("get_battery")
            ...
            #     #         self.check_battery_list = self.check_batterchecky_list[1:]
            self.__bat_num = _bat
    ...
    def get_time(self, event=None, msg=None):
        ...
        date = "{0}/{1:02d}/{2:02d}".format(str(local_time[0])[2:], local_time[1], local_time[2])
    ...
    def get_battery(self, event=None, msg=None):
        ...
        :return:
        """
        battery = round((self.__bat_num - 1) / 1000, 2)
        if battery < 0:
            battery_level = 9
        elif battery <= 3.4:
            battery_level = 0
            ...
            battery_level = 4
        if mdls.screen_size_style[mdls.models][2] == 1:
...
class LedManage(Abstract):
    ...
    def __init__(self):
        ...
        self.__red_led = Pin(mdls.model_gpio[gpio_type]["NET_MODE_LED"][0], Pin.OUT, Pin.PULL_DISABLE, 0)
        self.__blue_led = Pin(mdls.model_gpio[gpio_type]["NET_LED"][0], Pin.OUT, Pin.PULL_DISABLE, 0)
...
class MediaManager(Abstract):
    ...
    def __init__(self):
        ...
        self.log = LogAdapter(self.__class__.__name__)
        self.mic_det = Pin(mdls.model_gpio[gpio_type]["ej"][0], Pin.IN, Pin.PULL_PU, 0)
        # self.aud.setSpeakerpaCallback(self.speaker_pa_callback)
        self.__p18 = Pin(mdls.model_gpio[gpio_type]["AUDIO_PA_EN"][0], Pin.OUT, Pin.PULL_DISABLE, 0)
        self.__p18.write(0)
        if self.mic_det.read():
            self.aud.set_pa(mdls.model_gpio[gpio_type]["AUDIO_PA_EN"][0], 4)
            self.mic_sel = Pin(mdls.model_gpio[gpio_type]["mic"][0], Pin.OUT, Pin.PULL_DISABLE, 0)
        else:
            self.mic_sel = Pin(mdls.model_gpio[gpio_type]["mic"][0], Pin.OUT, Pin.PULL_DISABLE, 1)
        self.ext_mic_det = ExtInt(mdls.model_gpio[gpio_type]["ext_mic"][0], ExtInt.IRQ_RISING_FALLING, ExtInt.PULL_PU,
                                  self.cb)
        self.ext_mic_det.enable()
        if mdls.screen_size_style[mdls.models][1] == 2:
            self.vol_adc = ADC()
            _thread.start_new_thread(self.__vol_push_schedule, ())
        if mdls.screen_size_style[mdls.models][1] == 3:
            self.Knob_vol_a = Pin(Pin.GPIO3, Pin.IN, Pin.PULL_PU,1)
            self.Knob_vol_b = ExtInt(ExtInt.GPIO2, ExtInt.IRQ_RISING_FALLING, ExtInt.PULL_PU,self.__Knob_vol)
            self.Knob_vol_b.enable()
    ...
    def cb(self, ext_info):
        if ext_info[1]:
            ...
            self.aud.set_pa(mdls.model_gpio[gpio_type]["AUDIO_PA_EN"][0], 0)
        else:
    ...
    def post_processor_after_instantiation(self):
        ...
        EventMesh.subscribe("screen_vol_set", self.__adc_set_volume_level)
    ...
    def __adc_set_volume_level(self, event, msg):
        # print("__adc_set_volume_level:{}".format(msg))
        EventMesh.publish("lcd_state_manage")
        self.__set_volume(msg)
    ...
    def __set_volume(self, vol):
        """设置音量"""
        if vol != self.get_volume() and 0 <= vol <= mdls.screen_size_style[mdls.models][7]:
            ...
            if vol == 1 and self.get_volume() == 0 and self.mic_det.read():
                ...
                self.aud.set_pa(mdls.model_gpio[gpio_type]["AUDIO_PA_EN"][0], 4)
                print("启用结束")
            self.aud.setVolume(volume_level[vol])
    ...
    def get_volume(self):
        ...
        # return self.aud.getVolume()
        vol = 0
        for i, j in volume_level.items():
            if j == self.aud.getVolume():
                vol = i
        return vol
    ...
    # ADC vol
    def __vol_push_schedule(self, *args):
        new_vol = 0
        values = 0
        old_values =0
        while 1:
            if EventMesh.publish("get_lcd_sleep_mode") != 1:
                self.vol_adc.open()
                values = self.vol_adc.read(ADC.ADC0)
                # print("adc_vol_level:{}".format(adc_vol_level))
                # print("__vol_push_schedule:{}".format(values))
                if values >= 1190:
                    new_vol = adc_vol_level[1190]
                elif values >= 810:
                    new_vol = adc_vol_level[810]
                elif values >= 420:
                    new_vol = adc_vol_level[420]
                elif values >= 240:
                    new_vol = adc_vol_level[240]
                elif values >= 200:
                    new_vol = adc_vol_level[200]
                elif values >= 155:
                    new_vol = adc_vol_level[155]
                elif values >= 120:
                    new_vol = adc_vol_level[120]
                elif values >= 75:
                    new_vol = adc_vol_level[75]
                elif values >= 45:
                    new_vol = adc_vol_level[45]
                elif values >= 10:
                    new_vol = adc_vol_level[10]
                elif values <= 3:
                    new_vol = adc_vol_level[3]
                if new_vol[1] != self.get_volume():
                    EventMesh.publish("adc_volume_level", ["+", new_vol, self.get_volume(), values])
                self.vol_adc.close
            utime.sleep_ms(1000)

    def __Knob_vol(self,ext_info):
        ret = []
        ret2 = []
        for i in range(1):
            for i in range(1): pass # 延时10us
            ret.append(self.Knob_vol_b.read_level()) # 全为低电平 0
            ret2.append(self.Knob_vol_a.read())
        if ext_info[1]:
            if not(any(ret)):
                if all(ret2):
...
class PocManager(Abstract):
    ...
    def __init__(self):
        ...
        self.Version_No = mdls.models + "-" + mdls.modular + "-" + mdls.screen_size_style[mdls.models][10] + "-" + \
                          mdls.screen_size_style[mdls.models][11]
        self.platform_dict = {
            0: "xin",
            1: "std"
        }
        self.__net_sign = None
        self.__first_brush = 0
    ...
    def post_processor_after_instantiation(self, *args, **kwargs):
        ...
        EventMesh.subscribe("get_Version_No", self.get_Version_No)
        ...
        EventMesh.subscribe("get_first_brush", self.get_first_brush)
    ...
    def sync_setting_config(self):
        ...
        self.__net_sign = EventMesh.publish("persistent_config_get", "net_stype")
        self.__first_brush = EventMesh.publish("persistent_config_get", "first_brush")
        self.lower_power_start()
        self.ptt_tone_switch()

    def __set_net(self, event, msg):
        # print("__set_net:{}".format(self.__net_sign))

        if EventMesh.publish("persistent_config_get", "net_stype") == "4G":
            EventMesh.publish("persistent_config_store", {"net_stype": "5G"})
            self.__net_sign = "5G"
        else:
            EventMesh.publish("persistent_config_store", {"net_stype": "4G"})
            self.__net_sign = "4G"
    ...
    def get_first_brush(self, event, mode):
        return self.__first_brush

    def get_Version_No(self, event, mode):
    ...
    def __error_ptt_handler(self, params):
        ...
        if self.session_info:
            ...
            if not self.__rocker_arm:
                ...
                if m_weather_state == 0:
                    session_info = "话权空闲"   
                else:
                    session_info = "空闲"
        print("params[0] = {} error_msg = {}".format(params[0], self.error_msg))
        if params[0] == self.BND_LISTEN_STOP or self.error_msg == "内部会话状态错误" or self.error_msg == "抢麦时申请音频焦点失败":
            # 判断是否call状态结束
            if m_weather_state == 0:
                EventMesh.publish("update_session_info", "{}".format(session_info))
            else:
    ...
    def call_end(self, event, msg):
        ...
        if self.state:
            ...
            if not self.__rocker_arm:
                ...
                if m_weather_state == 0 :
                    EventMesh.publish("update_session_info", "话权空闲")
                else:
    ...
    def exit_call_member(self, *args):
        ...
        if self.__call_time_status:
            ...
            if group_cur and not group_cur[-2]:
                print("not Temporary groups not need exit {}".format(group_cur))
                return
            ...
            if group_cur[1] != self.about_user():
                ...
                if member_count > 1 or group_cur == -1:
    ...
    def request_lbs_info(self, event=None, msg=None):
        poc.request_lbs_info(m_width, m_height, 0.0, 0.0)
